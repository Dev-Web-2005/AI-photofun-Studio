      ...
      {!isOpen && (
        ...
        >
          <div className="relative">
            ...
            {/* Notification dot */}
            <span
              className={`absolute -top-1 -right-1 w-3 h-3 ${isDarkMode ? "bg-emerald-400" : "bg-emerald-500"
        ...
        >
          ...
          >
            <div className="flex items-center gap-3">
              <div
                className={`relative w-10 h-10 ${isDarkMode ? "bg-slate-700" : "bg-white"
                  } rounded-xl flex items-center justify-center shadow-sm group/icon`}
              >
                <Sparkles
                  className={`w-5 h-5 ${isDarkMode ? "text-slate-100" : "text-slate-900"
                    } group-hover/icon:rotate-12 transition-transform duration-300`}
                />
                <span
                  className={`absolute inset-0 rounded-xl opacity-0 group-hover/icon:opacity-100 transition-opacity duration-300 ${isDarkMode ? "bg-slate-600/20" : "bg-slate-100"
                    }`}
                ></span>
              </div>
              <div>
          ...
          >
            ...
            {messages.length === 1 && messages[0].id === 1 && (
              <div className="mt-3 animate-fade-in">
                <p
                  className={`text-xs font-medium mb-2 ${isDarkMode ? "text-slate-500" : "text-slate-400"
                    }`}
                >
                  💡 Ask me about:
                </p>
                  ...
                  {[
                    {
                      ...
                      prompts: [
                        ...
                        "How does Prompt to Image work?",
                        "How to create AI art?",
                        "What prompts work best?",
                        "How to get better image results?",
                      ],
                    },
                    {
                      icon: "🔍",
                      prompts: [
                        "How to upscale my images?",
                        "What does image enhancement do?",
                        "How to improve image quality?",
                        "How to make images HD?",
                        "What is the upscale feature?",
                      ],
                    },
                    {
                      icon: "🧹",
                      prompts: [
                        ...
                        "Can I make transparent images?",
                        "How to cut out subjects?",
                        "What is the remove BG tool?",
                      ],
                    },
                    {
                      icon: "🎭",
                      prompts: [
                        "How to change image style?",
                        "What is style transfer?",
                        "How to make anime style?",
                        "Can I apply art styles?",
                    ...
                    },
                    {
                      ...
                      prompts: [
                        ...
                        "Can I make videos from text?",
                        "How long are AI videos?",
                      ],
                    },
                  ].map((card, idx) => (
                    <button
                      ...
                      className={`text-left px-3 py-2 rounded-lg border transition-all duration-200 hover:scale-[1.01] ${isDarkMode
                          ? "bg-slate-800/50 border-slate-700 hover:bg-slate-700/50"
                          : "bg-white border-slate-200 hover:bg-slate-50"
                        }`}
                    >
                      <span className="text-sm mr-2">{card.icon}</span>
                      <span
                        className={`text-xs ${isDarkMode ? "text-slate-400" : "text-slate-500"
                          }`}
                      >
              ...
              <div className="flex justify-start animate-fade-in">
                ...
                >
                  <div className="flex gap-1.5">
                    <span
                      className={`w-2 h-2 ${isDarkMode ? "bg-slate-500" : "bg-slate-400"
